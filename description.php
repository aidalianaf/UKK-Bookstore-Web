...
if(!isset($_SESSION['user']))
       header("location: index.php?Message=Login Terlebih dahulu untuk dapat melanjutkan");
?>
        ...
        <div id="searchbox" class="container-fluid" style="width:112%;margin-left:-6%;margin-right:-6%;">
            <div>
                <form role="search" action="Result.php" method="post">
                    <input type="text" name="keyword" class="form-control" placeholder="Cari Judul Buku, Penulis atau Kategori Buku" style="width:80%;margin:20px 10% 20px 10%;">
                </form>
            </div>
        </div>
   </div>
...
echo '
   ...
   KODE PRODUK      '.$row["PID"].'   <hr> 
   JUDUL            '.$row["Title"].' <hr> 
   PENULIS          '.$row["Author"].' <hr>
   TERSEDIA         '.$row["Available"].' <hr> 
   PENERBIT         '.$row["Publisher"].' <hr> 
   EDISI            '.$row["Edition"].' <hr>
   BAHASA           '.$row["Language"].' <hr>
   JUMLAH HALAMAN   '.$row["page"].' <hr>
   BERAT            '.$row["weight"].' <hr>
                        </pre>
    </div>
  </div>
